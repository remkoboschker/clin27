...
module.exports = function Translator({
  ...
  function InVocab({ vocab }) {
    return async function inVocab(token) {
      return new Promise((resolve, reject) => {
        const word = token.get('word');
        vocab.get(`uni\x00${word}`, async (err) => {
          if (err) {
            if (err.notFound) {
              if (/^[A-Z].+/.test(word)) {
                const noCap = word.slice(0, 1).toLowerCase() + word.slice(1);
                resolve(await inVocab(token.set('word', noCap)));
              }
              resolve(false);
            } else {
              reject(err);
  ...
  function PossibleTranslations({ vocab, scaleDistance, preprocessWord, inVocab }) {
    ...
    return async function possibleTranslations(token) {
      ...
      const word = token.get('word');
      const inVocabulary = await inVocab(token);
      ...
      if (!changeIfInDict && inVocabulary) {
        return singleTranslation(token, token.get('word'), inVocabulary);
      }
  ...
  function MaximumPathToEachPossibleTranslation({ allPaths, maxPath }) {
    const translations = fs.createWriteStream(`${new Date().toISOString()}-${textName}-${vocabName}-${ngramName}-levSum-${levSum}-changeIfInDict-${changeIfInDict}-altProb-${altProb}.csv`);
  ...
  function TranslateText({ translateSentence, startOfSentenceMarker }) {
    const translated = fs.createWriteStream(`${new Date().toISOString()}-${textName}-${vocabName}-${ngramName}-levSum-${levSum}-changeIfInDict-${changeIfInDict}-altProb-${altProb}.txt`);
  ...
  async function translate() {
    ...
    return new Promise((reject, resolve) => {
      textDb.createReadStream({
        ...
        if (textCollector[sentenceNumber - 1] === undefined) {
          textCollector[sentenceNumber - 1] = [];
        }
        textCollector[sentenceNumber - 1][wordNumber - 1] = Immutable.Map({
          word: value,
          paragraphNumber,
          sentenceNumber,
          wordNumber,
